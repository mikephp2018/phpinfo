...
$logger->setLogPath('task_119506_1', "ibm/upgrade/ibm_r42/scripts/php/manual/");
...
if ($argc == 1) {
    ...
    COUNT(*) AS c
FROM
    ibm_roadmaps rm INNER JOIN ibm_roadmaps rmo
        ON rm.RLI_ID = rmo.RLI_ID
    AND rm.deleted = 0
    AND rmo.deleted = 0
    AND rmo.lio_own = 1
    AND rm.lio_own = 0
    AND rm.is_seller = 1
    AND rm.FORECASTING_ROLE != 'BPEMP'
    AND rm.BRAND_VERSION != 'GBP'
WHERE
    rm.related_to_roadmap_id <> rmo.id
